...
-- Desync Variables (NEW)
local desyncConn = nil
local desyncAnimConn = nil
...
-- Colors (Nexus theme - Black & White)
local C_BG = Color3.fromRGB(0, 0, 0)
local C_ACCENT = Color3.fromRGB(255, 255, 255)   -- White
local C_ACCENT2 = Color3.fromRGB(200, 200, 200)  -- Light gray
local C_DIM = Color3.fromRGB(120, 120, 120)      -- Gray
local C_SW_OFF = Color3.fromRGB(28, 28, 28)
local C_WHITE = Color3.fromRGB(255, 255, 255)
local C_ROW_BG = Color3.fromRGB(10, 10, 10)
...
local function startDesync()
    ...
    if hum then
        ...
        desyncAnimConn = hum.AnimationPlayed:Connect(function(track)
            if desyncEnabled then track:Stop() end
        end)
    end
...
local function stopDesync()
    desyncEnabled = false
    Enabled.Desync = false
    if desyncConn then
        desyncConn:Disconnect()
        desyncConn = nil
    end
    if desyncAnimConn then
        desyncAnimConn:Disconnect()
        desyncAnimConn = nil
    end
    if raknet and raknet.desync then
...
local function saveConfig()
    local cfg = {
        ...
        desyncKey = Keybinds.Desync.Name,
    ...
    if writefile then
        pcall(function()
            writefile("NexusHubConfig.json", HttpService:JSONEncode(cfg))
            ok = true
        end)
    end
...
local function loadConfig()
    if not (isfile and isfile("NexusHubConfig.json")) then return end
    local ok, cfg = pcall(function()
        return HttpService:JSONDecode(readfile("NexusHubConfig.json"))
    end)
    ...
    if cfg.desyncKey and Enum.KeyCode[cfg.desyncKey] then Keybinds.Desync = Enum.KeyCode[cfg.desyncKey] end
    ...
    if cfg.desyncEnabled ~= nil then desyncEnabled = cfg.desyncEnabled end
end
...
if true then -- mobile buttons (always enabled)
    ...
    local MARGIN = 10
    local col2X = vp.X - BTN_W - MARGIN
    local col1X = col2X - BTN_W - GAP
    local totalH = BTN_H*5 + GAP*4  -- increased for DESYNC
    ...
    -- COL 1: FLOAT, CARRY, BAT, DESYNC, UI (stacked top to bottom)
    ...
    createMobileButton("CARRY", col1X, startY + (BTN_H+GAP)*1, Color3.fromRGB(255, 150, 0), function(on)
        ...
        if speedSwBg and speedSwCircle then
            TweenService:Create(speedSwBg, TweenInfo.new(0.2), {BackgroundColor3 = on and C_ACCENT or C_SW_OFF}):Play()
    ...
    createMobileButton("BAT", col1X, startY + (BTN_H+GAP)*2, Color3.fromRGB(255, 50, 50), function(on)
        ...
        if batSwBg and batSwCircle then
            TweenService:Create(batSwBg, TweenInfo.new(0.2), {BackgroundColor3 = on and C_ACCENT or C_SW_OFF}):Play()
    ...
    -- NEW DESYNC BUTTON
    createMobileButton("DESYNC", col1X, startY + (BTN_H+GAP)*3, Color3.fromRGB(200, 0, 200), function(on)
        desyncEnabled = on
        Enabled.Desync = on
        if VisualSetters.Desync then VisualSetters.Desync(on) end
        if on then startDesync() else stopDesync() end
    end)

    createMobileButton("UI", col1X, startY + (BTN_H+GAP)*4, Color3.fromRGB(130, 130, 130), function(on)
        if main then main.Visible = not on end
    end)

    -- COL 2: A.LEFT, A.RIGHT, INF JMP, LOCK
    ...
    print("✓ MOBILE BUTTONS: 2 horizontal rows + DESYNC button")
end
...
local function makeSwitch(parent, enabledKey, defaultOn)
    ...
    swBg.BackgroundColor3 = defaultOn and C_ACCENT or C_SW_OFF
    ...
    local function setVisual(state)
        isOn = state
        TweenService:Create(swBg, TweenInfo.new(0.2, Enum.EasingStyle.Quint), {BackgroundColor3 = isOn and C_ACCENT or C_SW_OFF}):Play()
...
local function makeRow(keybindRefName, labelTxt, enabledKey, order, onToggle)
    ...
    row.Size = UDim2.new(1, 0, 0, isMobile and 54 or 44)
    ...
    if keybindRefName ~= "" then
        ...
        kbBox.BackgroundColor3 = C_ACCENT
    ...
    if enabledKey == "AutoSteal" then
        ...
        dropInner.BackgroundColor3 = Color3.fromRGB(10, 10, 10)
        ...
        dropInput.BackgroundColor3 = Color3.fromRGB(20, 20, 20)
        dropInput.BorderSizePixel = 0
        dropInput.Text = tostring(Values.STEAL_RADIUS)
        dropInput.TextColor3 = C_ACCENT
        ...
        dropInput.Focused:Connect(function()
            TweenService:Create(rStr, TweenInfo.new(0.15), {Color = C_ACCENT}):Play()
        end)
        ...
        durationInput.BackgroundColor3 = Color3.fromRGB(20, 20, 20)
        durationInput.BorderSizePixel = 0
        durationInput.Text = tostring(Values.STEAL_DURATION)
        durationInput.TextColor3 = C_ACCENT
        ...
        durationInput.Focused:Connect(function()
            TweenService:Create(dStr, TweenInfo.new(0.15), {Color = C_ACCENT}):Play()
        end)
    ...
    clk.Size = UDim2.new(1, 0, 0, isMobile and 54 or 44)
    ...
    clk.MouseEnter:Connect(function()
        playTick()
        TweenService:Create(nameLbl, TweenInfo.new(0.1), {TextColor3 = C_ACCENT2}):Play()
        TweenService:Create(row, TweenInfo.new(0.12), {BackgroundColor3 = Color3.fromRGB(18, 18, 18)}):Play()
    end)
    clk.MouseLeave:Connect(function()
        TweenService:Create(nameLbl, TweenInfo.new(0.1), {TextColor3 = C_ACCENT}):Play()
...
local _, setDesync = makeRow("Desync", "Desync", "Desync", O(), function(s)
    desyncEnabled = s
    Enabled.Desync = s
    if s then startDesync() else stopDesync() end
    if MobileButtons["DESYNC"] then MobileButtons["DESYNC"].setState(s) end
end)
VisualSetters.Desync = setDesync
...
speedClk.MouseButton1Click:Connect(function()
    ...
    TweenService:Create(speedSwBg, TweenInfo.new(0.2, Enum.EasingStyle.Quint), {BackgroundColor3 = speedToggled and C_ACCENT or C_SW_OFF}):Play()
...
batClk.MouseButton1Click:Connect(function()
    ...
    TweenService:Create(batSwBg, TweenInfo.new(0.2, Enum.EasingStyle.Quint), {BackgroundColor3 = autoBatToggled and C_ACCENT or C_SW_OFF}):Play()
...
local function makeInputBox(parent, lbl, defaultTxt, onDone)
    ...
    wrap.Size = UDim2.new(1, 0, 0, isMobile and 54 or 44)
    ...
    box.Focused:Connect(function()
        TweenService:Create(bs, TweenInfo.new(0.15), {Color = C_ACCENT}):Play()
    end)
...
local function makeSlider(parent, lbl, minVal, maxVal, defaultVal, onChanged)
    ...
    trackBg.BackgroundColor3 = Color3.fromRGB(14, 14, 14)
    ...
    trackFill.BackgroundColor3 = C_ACCENT
    ...
    local function updateSlider(inputPos)
        ...
        local thumbOffset = isMobile and -10 or -7
...
local function makeKeybindRow(parent, lbl, keybindRefName, onChanged)
    ...
    kbBtn.BackgroundColor3 = Color3.fromRGB(14, 14, 14)
    ...
    kbBtn.MouseButton1Click:Connect(function()
        ...
        if waitingForKey then waitingForKey.TextColor3 = C_ACCENT end
        waitingForKey = kbBtn
        kbBtn.Text = "..."
        kbBtn.TextColor3 = C_ACCENT2
        TweenService:Create(ks, TweenInfo.new(0.15), {Color = C_ACCENT}):Play()
    end)
...
do
    ...
    floatRow.Size = UDim2.new(1, 0, 0, isMobile and 54 or 44)
    ...
    fClk.Size = UDim2.new(1, 0, 0, isMobile and 54 or 44)
    ...
    local function setFloatVisual(on)
        floatEnabled = on
        TweenService:Create(fSwBg, TweenInfo.new(0.2, Enum.EasingStyle.Quint), {BackgroundColor3 = on and C_ACCENT or C_SW_OFF}):Play()
        ...
        TweenService:Create(floatLbl, TweenInfo.new(0.1), {TextColor3 = on and C_ACCENT2 or C_ACCENT}):Play()
    end
    ...
    fClk.MouseEnter:Connect(function()
        playTick()
        TweenService:Create(floatLbl, TweenInfo.new(0.1), {TextColor3 = C_ACCENT2}):Play()
        TweenService:Create(floatRow, TweenInfo.new(0.12), {BackgroundColor3 = Color3.fromRGB(18, 18, 18)}):Play()
    end)
    fClk.MouseLeave:Connect(function()
        if not floatEnabled then
            TweenService:Create(floatLbl, TweenInfo.new(0.1), {TextColor3 = C_ACCENT}):Play()
        end
...
makeKeybindRow(kbWrap, "Desync", "Desync", function(kc)
    Keybinds.Desync = kc
end)
...
saveConfigBtn.MouseEnter:Connect(function()
    TweenService:Create(saveConfigBtn, TweenInfo.new(0.1), {TextColor3 = C_ACCENT}):Play()
end)
...
ProgressLabel.TextColor3 = C_ACCENT
...
ProgressPercentLabel.TextColor3 = C_ACCENT
...
RadiusInput.BackgroundColor3 = Color3.fromRGB(14, 14, 14)
...
pTrack.BackgroundColor3 = Color3.fromRGB(14, 14, 14)
...
UserInputService.InputBegan:Connect(function(input, gpe)
    ...
    if input.KeyCode == Keybinds.Desync then
        desyncEnabled = not desyncEnabled
        Enabled.Desync = desyncEnabled
        if VisualSetters.Desync then VisualSetters.Desync(desyncEnabled) end
        if desyncEnabled then startDesync() else stopDesync() end
        if MobileButtons["DESYNC"] then MobileButtons["DESYNC"].setState(desyncEnabled) end
    end
...
task.spawn(function()
    ...
    if Enabled.Desync then
        desyncEnabled = true
        if VisualSetters.Desync then VisualSetters.Desync(true) end
        startDesync()
        if MobileButtons["DESYNC"] then MobileButtons["DESYNC"].setState(true) end
    end
    ...
    if MobileButtons then
        ...
        if MobileButtons["DESYNC"] and desyncEnabled then MobileButtons["DESYNC"].setState(true) end
...
print("✅ NEXUS HUB LOADED - WITH DESYNC FLOATING BUTTON")
print("✅ discord.gg/nexushub")
